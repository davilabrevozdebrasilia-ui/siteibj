...
function LazyImage({ src, alt }: { src: string; alt: string }) {
    ...
    return (
        <img
            ...
            src={visible ? src : ""}
            alt={alt}
            className={`max-w-full max-h-full object-contain transition-opacity duration-700 ${visible ? "opacity-100" : "opacity-0"
                }`}
        />
    );
}
...
export default function PaginaImagens() {
    ...
    const fetchImagens = async (pageNum: number) => {
        ...
        for (const img of novasImagens) {
            ...
            // permite que o React renderize antes de continuar
