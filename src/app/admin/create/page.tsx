...
export default function AdminCreate() {
    ...
    // Pega token do localStorage ao carregar o componente
    useEffect(() => {
    ...
    async function handleSubmitNoticia(e: React.FormEvent) {
        e.preventDefault();
        setError("");
        setSuccess("");
    ...
    async function handleSubmitAnuncio(e: React.FormEvent) {
        e.preventDefault();
        setError("");
        setSuccess("");
    ...
    if (!token) {
        return (
            <div className="max-w-md mx-auto mt-10 p-6 border rounded text-center">
                <p className="text-red-600 font-semibold mb-4">{error || "Acesso negado. Faça login para continuar."}</p>
                <button
    ...
    return (
        <div className="max-w-lg mx-auto mt-10 p-6 border rounded space-y-6">
            <div className="flex gap-4">
                <button
                    className={`flex-1 p-2 rounded ${aba === "noticia" ? "bg-green-700 text-white" : "bg-gray-200"
                        }`}
                    onClick={() => {
                        setError("");
                        setSuccess("");
                ...
                <button
                    className={`flex-1 p-2 rounded ${aba === "anuncio" ? "bg-green-700 text-white" : "bg-gray-200"
                        }`}
                    onClick={() => {
                        setError("");
                        setSuccess("");
            ...
            {aba === "noticia" && (
                <form onSubmit={handleSubmitNoticia} className="flex flex-col gap-4">
                    <input
                        type="text"
                        placeholder="Título"
            ...
            {aba === "anuncio" && (
                <form onSubmit={handleSubmitAnuncio} className="flex flex-col gap-4">
                    <input
                        type="text"
                        placeholder="Título"
